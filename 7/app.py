...
def run_evaluate():
    ...
    if os.path.exists(param_path):
        params = torch.load(param_path)
        net.load_state_dict(params)
    y, y_index, t_index, probs = evaluate_model(net, X, t)
    return jsonify({'y': y, 'y_index': y_index, 't_index': t_index, 'probs': probs})

# モデルの評価
def evaluate_model(network, X, t):
    y = network(X)
    y_index = torch.max(y, 1)[1]
    t_index = torch.max(t, 1)[1]
    y = y.tolist()
    probs = [v[y_index[i]] for i,v in enumerate(y)]
    return y, y_index.tolist(), t_index.tolist(), probs
